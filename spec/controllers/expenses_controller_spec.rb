...
RSpec.describe ExpensesController, type: :controller do
  ...
  let!(:group2) { Group.create(name: 'Group 2', icon: 'www.test2.com', user: user) }
  before(:each) do
    user.confirm
    sign_in user
  end
  ...
  describe 'POST #create' do
    context 'with valid params' do
      it 'creates a new expense and redirects to group page' do
        expect {
          post :create, params: { group_id: group.id, expense: { name: 'New Expense', amount: 100, group_ids: [group2.id] } }
